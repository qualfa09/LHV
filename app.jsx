...
// ==========================================
function ImagePreview({ file, targetWidth }) {
  ...
  if (file.type.startsWith('image/')) {
    return (
      <div style={{ marginTop: '10px' }}>
        ...
        <span style={{ fontSize: '11px', color: '#1565c0', display: 'block', marginTop: '5px', fontWeight: 'bold' }}>
          ↔️ Output Word dikunci: {targetWidth}
        </span>
      </div>
    );
  }
    ...
    <div style={{ ...sectionStyle, backgroundColor: '#fff' }}>
      ...
      {state.map((item, index) => (
        <div key={item.id} style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '12px', paddingBottom: '12px', borderBottom: '1px dashed #eee' }}>
          ...
          {/* Pratinjau Dokumen Umum dikunci pada 7 cm */}
          <ImagePreview file={item.file} targetWidth="7 cm" />
        </div>
      ))}
                ...
                <div key={form.id} style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '15px', border: '1px solid #e0e0e0', borderRadius: '6px', backgroundColor: '#fafafa' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    ...
                    <div style={{ flex: '2' }}>
                      ...
                      <input type="file" accept="image/png, image/jpeg" onChange={(e) => setFormulirVerifikasi(formulirVerifikasi.map(f => f.id === form.id ? { ...f, file: e.target.files[0] } : f))} style={{ width: '100%', marginTop: '8px' }} />
                    </div>
